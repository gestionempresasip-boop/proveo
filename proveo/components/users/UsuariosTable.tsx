...
function PinCell({ user }: { user: UserRow }) {
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={() => { setEditing(false); setError(null) }} className="p-1 rounded text-gray-400 hover:bg-gray-100"><X className="w-4 h-4" /></button>
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="font-mono text-sm text-[#1C1C1E] tabular-nums">
        {user.pin ? (visible ? user.pin : '••••') : <span className="text-gray-300">Sin PIN</span>}
      </span>
      {user.pin && (
        <button onClick={() => setVisible(v => !v)} className="p-1 rounded text-gray-400 hover:text-[#1E2B28] hover:bg-gray-100">
          {visible ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
        </button>
      )}
      <button onClick={() => setEditing(true)} title="Cambiar PIN" className="p-1 rounded text-gray-400 hover:text-[#1E2B28] hover:bg-gray-100">
...
function NameCell({ user }: { user: UserRow }) {
  ...
  if (editing) {
    return (
      <form onSubmit={handleSubmit} className="flex items-center gap-1.5">
        ...
        <button type="button" onClick={() => setEditing(false)} className="p-1 rounded text-gray-400 hover:bg-gray-100"><X className="w-4 h-4" /></button>
      </form>
    )
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-medium text-[#1C1C1E]">{user.full_name ?? 'Sin nombre'}</span>
      <button onClick={() => setEditing(true)} title="Editar" className="p-1 rounded text-gray-300 hover:text-[#1E2B28] hover:bg-gray-100">
        ...
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-gray-500 font-medium">PIN de acceso</th>
            </tr>
          </thead>
