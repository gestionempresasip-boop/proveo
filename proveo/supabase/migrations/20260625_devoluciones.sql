-- Devoluciones de mercancía ya entregada. Se modelan como un albarán más
-- (delivery_notes) con type = 'devolucion', enlazado al mismo pedido, para
-- reaprovechar toda la UI/RLS de albaranes ya existente. Cada línea indica
-- si el producto vuelve a sumar a stock de la nave (reutilizable) o no
-- (mal estado / rotura / no_utilizable).

ALTER TABLE delivery_notes ADD COLUMN IF NOT EXISTS type text NOT NULL DEFAULT 'entrega'
  CHECK (type IN ('entrega', 'devolucion'));

ALTER TABLE delivery_note_items ADD COLUMN IF NOT EXISTS return_reason text
  CHECK (return_reason IN ('reutilizable', 'no_utilizable'));

-- Crea una devolución completa (cabecera + líneas) y, para las líneas
-- marcadas como reutilizables, devuelve esa cantidad al stock de la nave.
-- SECURITY DEFINER porque los restaurantes no tienen permiso de INSERT
-- sobre delivery_notes/delivery_note_items (solo la nave), pero sí deben
-- poder registrar una devolución de SU PROPIO pedido ya entregado.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- No se puede devolver más de lo que realmente se entregó (descontando
    -- devoluciones previas ya registradas para ese mismo producto/pedido).
...
REVOKE ALL ON FUNCTION create_return(uuid, jsonb) FROM public;
GRANT EXECUTE ON FUNCTION create_return(uuid, jsonb) TO authenticated;
